<template>
    <div class="header-wrapper d-flex pb-3 flex-column flex-xl-row gap-3 justify-content-between align-items-center">
        <div class="headers d-flex gap-3 align-items-center col-12 col-xl-auto">
            ...
            <a-switch
                v-if="$rootAccess || $moderAccess"
                :checked-children="$locale.userProfilePage.removedServicesSwitch.checked"
                :un-checked-children="$locale.userProfilePage.removedServicesSwitch.unchecked"
                v-model:checked="showRemovedServices" @change="refreshServices"
            />
        </div>
        <div class="actions-wrapper d-flex flex-column flex-xl-row gap-3 gap-xl-2 col-12 col-xl-6">
            ...
            <div v-if="$rootAccess || $moderAccess" class="btn-wrapper">
                <a-button :block="$windowWidth <= 1200" type="primary" size="large" @click="showCreateModal">
                    {{ $locale.userProfilePage.addServiceButtonTitle }}
                </a-button>
            </div>
        </div>
    </div>
    <div class="table-wrapper">
        <a-table v-if="$mobile" :row-key="record => record.id" table-layout="auto" bordered :data-source="services"
                 :columns="columns" :pagination="{ pageSize: 1000 }" :scroll="{ y: 327 }"
                 :custom-row="customServicesRow">
            ...
            <template #operation="{ record }">
                <div class="btn-wrapper d-flex justify-content-end gap-2">
                    <a-button type="primary" @click="() => showEditModal(record)">
                        {{ $locale.homePage.servicesTableEditButtonTitle }}
                    </a-button>
                    <a-popconfirm
        ...
        >
            <div class="input-wrapper d-flex flex-column align-items-start py-2">
                ...
                <a-input size="large" :disabled="editModalConfirmLoading" class="w-100"
                         v-model:value="editModalState.title" @pressEnter="saveService"/>
            </div>
            <div class="input-wrapper d-flex flex-column align-items-start py-2">
                <h6>{{ $locale.homePage.servicesTableHeaders.price }}</h6>
                <a-input-number size="large" :disabled="editModalConfirmLoading" type="number" pattern="[0-9]*"
                                inputmode="numeric" class="w-100" v-model:value="editModalState.price" :min="1"
                                @pressEnter="saveService"/>
            </div>
            <div class="input-wrapper d-flex flex-column align-items-start py-2">
                <h6>{{ $locale.homePage.servicesTableHeaders.description }}</h6>
                <a-input size="large" :disabled="editModalConfirmLoading" class="w-100"
                         v-model:value="editModalState.description" @pressEnter="saveService"/>
            </div>
            <div class="input-wrapper d-flex flex-column align-items-start py-2">
                <h6>{{ $locale.homePage.servicesTableHeaders.customerDescription }}</h6>
                <a-input size="large" :disabled="editModalConfirmLoading" class="w-100"
                         v-model:value="editModalState.customerDescription" @pressEnter="saveService"/>
            </div>
        </a-modal>
        <a-modal
        ...
        >
            <div class="input-wrapper d-flex flex-column align-items-start py-2">
                ...
                <a-input size="large" required :disabled="createModalConfirmLoading" class="w-100"
                         v-model:value="createModalState.title" @pressEnter="createService"/>
            </div>
            <div class="input-wrapper d-flex flex-column align-items-start py-2">
                <h6>{{ $locale.homePage.servicesTableHeaders.price }}</h6>
                <a-input-number size="large" :disabled="createModalConfirmLoading" type="number" pattern="[0-9]*"
                                inputmode="numeric" class="w-100" v-model:value="createModalState.price" :min="1"
                                @pressEnter="createService"/>
            </div>
            <div class="input-wrapper d-flex flex-column align-items-start py-2">
                <h6>{{ $locale.homePage.servicesTableHeaders.description }}</h6>
                <a-input size="large" :disabled="createModalConfirmLoading" class="w-100"
                         v-model:value="createModalState.description" @pressEnter="createService"/>
            </div>
            <div class="input-wrapper d-flex flex-column align-items-start py-2">
                <h6>{{ $locale.homePage.servicesTableHeaders.customerDescription }}</h6>
                <a-input size="large" :disabled="createModalConfirmLoading" class="w-100"
                         v-model:value="createModalState.customerDescription" @pressEnter="createService"/>
            </div>
            <a-alert
                class="mt-2"
                size="large"
...
export default defineComponent({
    ...
    setup() {
        ...
        const services = computed(
            () => showRemovedServices.value && (rootAccess.value || moderAccess.value)
        ...
        const showEditModal = (record: Service) => {
            ...
            editModalState.value.customerDescription = record.customerDescription ?? '';
        ...
        const setDefaultEditModalStateService = () => {
            ...
            editModalState.value.customerDescription = '';
        }
        ...
        const saveService = async () => {
            ...
            setDefaultEditModalStateService();
        ...
        const createService = async () => {
            ...
            createModalState.value.customerDescription = '';
        ...
        const columns = computed(() => {
            return [
                ...
                rootAccess.value || moderAccess.value
                    ? {
                        title: locale.value.homePage.servicesTableHeaders.customerDescription,
